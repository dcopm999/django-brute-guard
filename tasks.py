...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-brute-guard runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-brute-guard.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ bruteguard")
...
def lint(c):
    ...
    """
    c.run("flake8 django-brute-guard tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=django_brute_guard.__version__
        )
    )
    c.run("git push --tags")
